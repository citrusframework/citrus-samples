...
public class TodoListIT extends TestNGCitrusTestDesigner {
    ...
    public void testStoreAndRetrieveFile() throws IOException {
        ...
        send(ftpClient)
                .fork(true)
                .message(FtpMessage.get("todo/entry.json", DataType.ASCII).arguments("todo/entry.json"));

        receive(ftpServer)
                .message(FtpMessage.command(FTPCmd.RETR).arguments("todo/entry.json"));

        send(ftpServer)
                .payload(FtpMessage.success().getPayload(String.class));

        receive(ftpClient)
                .message(FtpMessage.result(getRetrieveFileCommandResult("todo/entry.json", new ClassPathResource("todo/entry.json"))));
    }
