...
public class SupplyApiResource {

    private static final Logger LOG = Logger.getLogger(SupplyApiResource.class);
    ...
    @GET
    @Operation(operationId = "listSupplies")
    public List<Supply> list() {
        return supplyService.findAll();
    }

    @POST
    @Operation(operationId = "addSupply")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response add(Supply supply) throws JsonProcessingException {
    ...
    @GET
    @Path("/{id}")
    @Operation(operationId = "getSupplyById")
    public Response find(@PathParam("id") String id) {
        ...
        try {
            return Response.ok(supplyService.findById(Long.valueOf(id))).build();
    ...
    @DELETE
    @Path("/{id}")
    @Operation(operationId = "deleteSupply")
    public Response delete(@PathParam("id") String id) {
        if (!Pattern.compile("\\d+").matcher(id).matches()) {
            return Response.status(Response.Status.BAD_REQUEST).build();
        }

        if (supplyService.remove(Long.valueOf(id))) {
